"""project_vacancy_skills: skills per vacancy for matching

Revision ID: proj_vac_skills_01
Revises: sprint4_app_employer_init
"""
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("vacancy_id", sa.Integer(), sa.ForeignKey("project_vacancies.id", ondelete="CASCADE"), nullable=False),
        sa.Column("skill_id", sa.Integer(), sa.ForeignKey("skills.id", ondelete="RESTRICT"), nullable=False),
    )
    op.create_unique_constraint(
        "uq_project_vacancy_skill",
        "project_vacancy_skills",
        ["vacancy_id", "skill_id"],
    )
