...
from src.enums import CommitmentLevel, ContractType, ProjectVacancyExperience, SalaryType, Schedule, WorkFormat
...
class ProjectVacancyRequestAdd(_ProjectVacancyBody):
    ...
    def _skills_norm(cls, v: list[int]) -> list[int]:
        ...
        if len(out) < 1:
            raise ValueError('В вакансии должен быть хотя бы один навык')
        return out
