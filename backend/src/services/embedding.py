...
def embed_query(text: str) -> list[float]:
    """Для резюме — запросы."""
    from src.embedding_provider import encode_query
...
async def recompute_resume_embedding_db(db: DBManager, resume_id: int) -> bool:
    ...
    # flush закрывает курсор pg_insert без завершения транзакции —
    # это позволяет следующим SELECT-ам работать без ошибки asyncpg
