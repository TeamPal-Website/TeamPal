...
from src.schemas.applications import (
    Application,
    ApplicationForOwner,
    ApplicationForOwnerDetail,
    ApplicationResumeForOwner,
...
class ApplicationsRepository(BaseRepository):
    ...
    async def get_with_context(
        ...
        query = (
            ...
            .join(OwnerProf, OwnerProf.id == ProjectsOrm.profile_id)
            .join(
                RolesDictionaryOrm,
                RolesDictionaryOrm.id == ProjectVacancyOrm.role_type_id,
            )
            .where(ApplicationsOrm.id == application_id)
        )
        ...
        return ApplicationWithContext(
            ...
            employer_initiated=bool(app.employer_initiated),
    ...
    async def get_my_applications(
        ...
        OwnerProf = aliased(ProfilesOrm)
        filters = [
            ResumesOrm.profile_id == profile_id,
        ...
        return [
            ...
            for app, project_id, project_title, role_type_id, role_name, project_owner_user_id in result.all()
        ]

    async def count_my_pending(self, profile_id: int) -> int:
        query = (
            select(func.count(ApplicationsOrm.id))
            .join(ResumesOrm, ResumesOrm.id == ApplicationsOrm.resume_id)
            .join(ProjectVacancyOrm, ProjectVacancyOrm.id == ApplicationsOrm.vacancy_id)
            .join(ProjectsOrm, ProjectsOrm.id == ProjectVacancyOrm.project_id)
            .where(
    ...
    async def count_incoming_pending_for_owner(self, owner_profile_id: int) -> int:
        query = (
            ...
            .join(ProjectsOrm, ProjectsOrm.id == ProjectVacancyOrm.project_id)
            .where(
                ProjectsOrm.profile_id == owner_profile_id,
                ApplicationsOrm.status == ApplicationStatus.PENDING,
            )
        )
        result = await self.session.execute(query)
    ...
    async def get_detail_for_owner(
        self, application_id: int
    ) -> ApplicationForOwnerDetail | None:
        query = (
            select(
                ApplicationsOrm,
                ProfilesOrm.id.label("applicant_profile_id"),
                ProfilesOrm.user_id.label("applicant_user_id"),
                ProfilesOrm.contacts,
                ResumesOrm.desired_position,
                ResumesOrm.about_me,
                ResumesOrm.employment_intent,
                ResumesOrm.commitment_level,
                ResumesOrm.work_format,
                ResumesOrm.schedule,
                ResumesOrm.salary_amount,
                ResumesOrm.salary_type,
                ResumesOrm.contract_type,
        ...
        return ApplicationForOwnerDetail(
            id=app.id,
            resume_id=app.resume_id,
            vacancy_id=app.vacancy_id,
            status=app.status,
    ...
    async def cancel_pending_for_resume(
        self, resume_id: int, reason: CancelReason
    ) -> list[int]:
        query = select(ApplicationsOrm.id).where(
            ApplicationsOrm.resume_id == resume_id,
            ApplicationsOrm.status == ApplicationStatus.PENDING,
        )
        result = await self.session.execute(query)
    ...
    async def cancel_pending_for_project(
    ...
    async def get_owner_user_id_for_application(
        self, application_id: int
    ) -> int | None:
        query = (
            select(ProfilesOrm.user_id)
            .join(ProjectsOrm, ProjectsOrm.profile_id == ProfilesOrm.id)
            .join(ProjectVacancyOrm, ProjectVacancyOrm.project_id == ProjectsOrm.id)
            .join(ApplicationsOrm, ApplicationsOrm.vacancy_id == ProjectVacancyOrm.id)
            .where(ApplicationsOrm.id == application_id)
        )
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def get_applicant_user_id_for_application(
        self, application_id: int
    ) -> int | None:
        query = (
            select(ProfilesOrm.user_id)
            .join(ResumesOrm, ResumesOrm.profile_id == ProfilesOrm.id)
            .join(ApplicationsOrm, ApplicationsOrm.resume_id == ResumesOrm.id)
            .where(ApplicationsOrm.id == application_id)
        )
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def has_blocking_application_for_user_vacancy(
        self, user_id: int, vacancy_id: int
    ) -> bool:
        exists_query = (
            select(ApplicationsOrm.id)
            .join(ResumesOrm, ResumesOrm.id == ApplicationsOrm.resume_id)
            .join(ProfilesOrm, ProfilesOrm.id == ResumesOrm.profile_id)
            .where(
                ProfilesOrm.user_id == user_id,
                ApplicationsOrm.vacancy_id == vacancy_id,
                ApplicationsOrm.status.in_(
                    [
