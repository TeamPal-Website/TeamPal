...
async def get_notifications(db: DBDep, user_id: UserIdDep, page: PageDep=1, per_page: PerPageDep=50):
    return await db.notifications.get_for_user(user_id=user_id, limit=per_page, offset=per_page * (page - 1))
...
async def mark_notifications_read(db: DBDep, user_id: UserIdDep, data: NotificationsMarkRead):
    if data.mark_all:
        ...
        await db.notifications.mark_read_by_ids(user_id=user_id, notification_ids=data.notification_ids)
    else:
        raise HTTPException(status_code=422, detail='Укажите notification_ids или mark_all=true')
    await db.commit()
    return {'status': 'OK'}
