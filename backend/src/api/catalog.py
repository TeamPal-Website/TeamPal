...
from src.enums import CommitmentLevel, EmploymentIntent, ProjectVacancyExperience, ProjectsStatus, ResumeStatus, SalaryType, Schedule, WorkFormat
...
async def catalog_vacancies(db: DBDep, user_id: UserIdDep, city_id: int | None=None, employment_intent: EmploymentIntent | None=None, role_type_id: int | None=None, experience: ProjectVacancyExperience | None=None, work_format: WorkFormat | None=None, schedule: Schedule | None=None, commitment_level: CommitmentLevel | None=None, salary_type: SalaryType | None=None):
    ...
    for vacancy, project in rows:
        out.append({'vacancy': ProjectVacancy.model_validate(vacancy, from_attributes=True), 'project': Project.model_validate(project, from_attributes=True)})
    return out

async def _can_view_project(db, project: ProjectsOrm, viewer_profile_id: int) -> bool:
    ...
    if project.status == ProjectsStatus.CLOSE:
        if project.profile_id == viewer_profile_id:
            return True
        acl_q = select(ProjectCloseAclOrm).where(ProjectCloseAclOrm.project_id == project.id, ProjectCloseAclOrm.profile_id == viewer_profile_id)
...
async def catalog_vacancy_detail(vacancy_id: int, db: DBDep, user_id: UserIdDep):
    ...
    if not await _can_view_project(db, project, profile.id):
        raise HTTPException(status_code=404, detail='Вакансия не найдена')
    filled_sub = exists(select(VacancyAssignmentOrm.id).where(VacancyAssignmentOrm.vacancy_id == vacancy_id, VacancyAssignmentOrm.released_at.is_(None)))
    filled = await db.session.scalar(select(filled_sub))
    slot_open = not bool(filled)
    return {'vacancy': ProjectVacancy.model_validate(vacancy, from_attributes=True), 'project': Project.model_validate(project, from_attributes=True), 'slot_open': bool(slot_open)}

@router.get('/catalog/resumes')
async def catalog_resumes(db: DBDep, user_id: UserIdDep, employment_intent: EmploymentIntent | None=None, experience_band: ProjectVacancyExperience | None=None, work_format: WorkFormat | None=None, schedule: Schedule | None=None, commitment_level: CommitmentLevel | None=None, salary_type: SalaryType | None=None, city_id: int | None=Query(default=None)):
    profile = await db.profiles.get_one_or_none(user_id=user_id)
    if profile is None:
        raise HTTPException(status_code=404, detail='Профиль не найден')
