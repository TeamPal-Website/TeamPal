...
async def get_project(
    ...
    if project.status == ProjectsStatus.CLOSE:
        requesting_profile = await db.profiles.get_one_or_none(
            user_id=requesting_user_id
        )
        is_owner = (
            requesting_profile is not None and requesting_profile.id == profile.id
        )
        is_member = (
            project.close_member_ids is not None
            and requesting_user_id in project.close_member_ids
        )
        if not is_owner and not is_member:
            raise HTTPException(
                status_code=403, detail="Нет доступа к закрытому проекту"
            )
...
async def get_project_by_id(
    ...
    if project.status == ProjectsStatus.CLOSE:
        is_member = (
            project.close_member_ids is not None
            and user_id in project.close_member_ids
        )
        if not is_owner and not is_member:
            raise HTTPException(
                status_code=403, detail="Нет доступа к закрытому проекту"
...
async def close_project(
    ...
    if project.status != ProjectsStatus.ACTIVE:
        raise HTTPException(
            status_code=409, detail="Закрыть можно только активный проект"
        )
...
async def remove_member(
    ...
    if project is None or project.status in (
        ProjectsStatus.DELETED,
        ProjectsStatus.CLOSE,
    ):
        raise HTTPException(status_code=404, detail="Проект не найден")
