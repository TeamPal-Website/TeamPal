...
@router.get(
    '/profiles/{user_id}/projects/{project_id}',
    summary='Проект пользователя',
    description='Возвращает проект и вакансии по идентификатору владельца. Для закрытых проектов проверяется доступ просматривающего.',
)
...
@router.get(
    '/profiles/{user_id}/projects',
    summary='Активные проекты пользователя',
    description='Список активных проектов профиля по user_id.',
)
...
@router.get(
    ...
    summary='Мой проект',
    description='Возвращает один принадлежащий пользователю проект с вакансиями и участниками.',
)
...
@router.get(
    '/my_projects',
    summary='Мои открытые проекты',
    description='Список проектов текущего пользователя в статусах active и paused.',
)
...
@router.get(
    '/my_closed_projects',
    summary='Мои закрытые проекты',
    description='Список проектов текущего пользователя в статусе close.',
)
...
@router.get(
    ...
    summary='Закрытые проекты, где я участник',
    description='Список закрытых проектов, в которых пользователь участвовал как член команды.',
)
...
@router.get(
    '/projects',
    response_model=list[ProjectSearchItem],
    summary='Публичный поиск проектов',
    description='Поиск активных проектов с текстовым запросом, пагинацией и фильтрами по городу, типу и роли.',
)
...
@router.get(
    '/projects/{project_id}',
    summary='Проект по идентификатору',
    description='Возвращает проект и вакансии с проверкой доступа для paused и closed статусов.',
)
...
@router.post(
    ...
    description='Создаёт проект с вакансиями для текущего пользователя. Учитывается лимит active/paused проектов и полнота профиля.',
)
...
@router.patch(
    ...
    description='Частичное обновление полей проекта. При паузе отменяются pending-отклики, если нет участников.',
)
...
@router.post(
    ...
    summary='Закрытие проекта',
    description='Закрывает активный проект после заполнения всех вакансий. Снимает участников и отменяет открытые отклики.',
)
...
@router.delete(
    ...
    description='Мягкое удаление проекта без активных участников. Pending-отклики отменяются.',
)
...
@router.delete(
    '/projects/{project_id}/members/{resume_id}',
    summary='Исключение участника',
    description='Владелец снимает участника с вакансии, возвращает резюме в статус поиска работы и отменяет связанный отклик.',
)
