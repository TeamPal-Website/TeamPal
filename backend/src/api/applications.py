...
from src.schemas.applications import (
    ApplicationCreate,
    ApplicationAdd,
    VacancyAssignmentAdd,
    EmployerInviteResume,
    ApplicationsBadgeCounts,
)
...
async def applications_badge_counts(
    db: DBDep,
    user_id: UserIdDep,
):
    ...
    return ApplicationsBadgeCounts(
...
async def employer_invite_resume(
    vacancy_id: int,
    db: DBDep,
    user_id: UserIdDep,
    data: EmployerInviteResume,
):
    ...
    if await db.applications.has_blocking_application_for_user_vacancy(
    ...
    application = await db.applications.add(
        ApplicationAdd(
            resume_id=data.resume_id,
            vacancy_id=vacancy_id,
            employer_initiated=True,
        )
    )
    ...
    await db.notifications.create_notification(
        NotificationAdd(
            user_id=applicant_profile.user_id,
            event=NotificationEvent.EMPLOYER_INVITED,
...
async def create_application(
    ...
    application = await db.applications.add(
        ApplicationAdd(resume_id=data.resume_id, vacancy_id=data.vacancy_id)
    )
...
async def withdraw_application(
    ...
    resume = await db.resumes.get_one_or_none(
        id=application.resume_id, profile_id=profile.id
    )
    if resume is None:
    ...
    if application.status == ApplicationStatus.PENDING:
        await db.applications.set_status(
            ...
            CancelReason.USER_WITHDRAWN,
        )
    elif application.status == ApplicationStatus.ACCEPTED:
        assignment = await db.vacancy_assignments.get_active_by_resume(
...
async def list_project_applications(
    ...
    applications = await db.applications.get_for_project_owner(
...
async def get_project_applications_new_count(
    project_id: int,
    db: DBDep,
    user_id: UserIdDep,
):
    ...
    count = await db.applications.count_new_for_project(
        project_id=project_id,
        last_seen_at=project.last_seen_applications_at,
    )
...
async def accept_application(
    ...
    project = await db.projects.get_one_or_none(
        id=vacancy.project_id, profile_id=profile.id
    )
    if project is None:
        raise HTTPException(status_code=403, detail="Нет доступа")
    ...
    await db.vacancy_assignments.add(
        VacancyAssignmentAdd(
            resume_id=application.resume_id,
            vacancy_id=application.vacancy_id,
            application_id=application_id,
    ...
    cancelled_ids = await db.applications.cancel_pending_for_resume(
        application.resume_id, CancelReason.ANOTHER_ACCEPTED
    )

    applicant_user_id = await db.applications.get_applicant_user_id_for_application(
        application_id
    )
    role = await db.roles_dictionary.get_one_or_none(id=vacancy.role_type_id)
    ...
    if applicant_user_id is not None:
        await db.notifications.create_notification(
            NotificationAdd(
                ...
                event=NotificationEvent.APPLICATION_ACCEPTED,
                application_id=application_id,
                project_id=project.id,
                payload={
                    "project_id": project.id,
    ...
    for cancelled_id in cancelled_ids:
        ...
        if cancelled_app is None:
            continue
        cancelled_applicant_uid = (
            await db.applications.get_applicant_user_id_for_application(cancelled_id)
        )
        if cancelled_applicant_uid is not None:
            cancelled_vacancy = await db.project_vacancies.get_one_or_none(
                id=cancelled_app.vacancy_id
            )
            if cancelled_vacancy is not None:
                cancelled_project = await db.projects.get_one_or_none(
                    id=cancelled_vacancy.project_id
                )
                if cancelled_project is not None:
                    await db.notifications.create_notification(
                        NotificationAdd(
                            user_id=cancelled_applicant_uid,
                            event=NotificationEvent.APPLICATION_CANCELLED,
                            application_id=cancelled_id,
                            project_id=cancelled_project.id,
                            payload={
                                "project_id": cancelled_project.id,
                                "project_title": cancelled_project.title,
                                "resume_id": cancelled_app.resume_id,
                                "vacancy_id": cancelled_app.vacancy_id,
                            },
                        )
                    )
...
async def accept_invitation_as_applicant(
    application_id: int,
    db: DBDep,
    user_id: UserIdDep,
):
