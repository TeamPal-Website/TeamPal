...
from src.schemas.profiles import ProfileAvatarKeyUpdate, ProfileRead, ProfileRequestPatch, PublicProfile
from src.services.object_storage import delete_avatar_key, get_object_bytes, is_object_storage_configured, upload_avatar
...
async def get_my_avatar_file(db: DBDep, user_id: UserIdDep):
    ...
    loaded = await get_object_bytes(res.avatar)
    if loaded is None:
        raise HTTPException(status_code=404, detail='Файл аватара не найден в хранилище')
