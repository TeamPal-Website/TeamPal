...
def _merge_resume_search_skill_ids(*, skill_id: int | None, skill_ids: list[int] | None) -> list[int] | None:
    ...
    if len(merged) > RESUME_SEARCH_SKILL_IDS_MAX:
        raise HTTPException(status_code=422, detail=f'Можно указать не более {RESUME_SEARCH_SKILL_IDS_MAX} навыков в фильтре')
    return merged
...
async def _expand_resume_role_patch_fields(db: DBDep, payload: dict) -> dict:
    ...
    if 'desired_position' in payload:
        ...
        if role is None:
            raise HTTPException(status_code=422, detail='Желаемая должность должна совпадать с записью в справочнике')
...
async def get_resume(db: DBDep, user_id: int, resume_id: int, viewer_id: OptionalViewerIdDep):
    ...
    resume = await db.resumes.get_one_or_none(id=resume_id, profile_id=profile.id, status=ResumeStatus.LOOKING_FOR_JOB)
    if resume is None:
...
async def search_resumes(db: DBDep, q: SearchQDep, page: PageDep=1, per_page: PerPageDep=10, city_id: int | None=Query(default=None, gt=0), employment_intent: EmploymentIntent | None=None, skill_id: int | None=Query(default=None, gt=0), skill_ids: list[int] | None=Query(default=None), role_type_id: int | None=Query(default=None, gt=0), work_format: WorkFormat | None=None, commitment_level: CommitmentLevel | None=None, salary_min: int | None=Query(default=None, ge=0), salary_max: int | None=Query(default=None, ge=0), computed_experience_level: ProjectVacancyExperience | None=None, created_within_days: int | None=Query(default=None, ge=1, le=366)):
    if salary_min is not None and salary_max is not None and (salary_min > salary_max):
        raise HTTPException(status_code=422, detail='Минимальная зарплата не может быть больше максимальной')
    merged_skill_ids = _merge_resume_search_skill_ids(skill_id=skill_id, skill_ids=skill_ids)
    return await db.resumes.search_public(q=q, city_id=city_id, employment_intent=employment_intent, skill_ids=merged_skill_ids, role_type_id=role_type_id, work_format=work_format, commitment_level=commitment_level, salary_min=salary_min, salary_max=salary_max, computed_experience_level=computed_experience_level, created_within_days=created_within_days, limit=per_page, offset=per_page * (page - 1))
...
async def create_resume(db: DBDep, user_id: UserIdDep, resume_data: ResumeRequestAdd):
    ...
    resume = await db.resumes.add(ResumeAdd(profile_id=profile.id, desired_position=role.name, role_type_id=role.id, city_id=resume_data.city_id, employment_intent=resume_data.employment_intent, commitment_level=resume_data.commitment_level, work_format=resume_data.work_format, schedule=resume_data.schedule, salary_amount=resume_data.salary_amount, salary_type=resume_data.salary_type, contract_type=resume_data.contract_type, about_me=resume_data.about_me, status=resume_data.status))
    experiences = []
    for experience_data in resume_data.experiences:
        role = await _require_active_role(db, experience_data.role_type_id)
        experience = await db.resume_experiences.add(ResumeExperienceAdd(resume_id=resume.id, company_name=experience_data.company_name, role_type_id=role.id, position=role.name, description=experience_data.description, start_date=experience_data.start_date, end_date=experience_data.end_date))
...
async def update_resume(resume_id: int, db: DBDep, user_id: UserIdDep, data: ResumePatch):
    ...
    for application_id in cancelled_ids:
        ...
        if project is None:
            continue
        await db.notifications.create_notification(NotificationAdd(user_id=owner_user_id, event=NotificationEvent.APPLICATION_CANCELLED, application_id=application_id, project_id=project.id, payload={'project_id': project.id, 'project_title': project.title, 'resume_id': resume_id, 'vacancy_id': application.vacancy_id, 'reason': 'resume_updated'}))
    res = await db.resumes.edit(data, exclude_unset=True, id=resume_id, profile_id=profile.id)
    if res == 0:
        raise HTTPException(status_code=404, detail='Резюме не найдено')
    await db.commit()
...
async def delete_resume(resume_id: int, db: DBDep, user_id: UserIdDep):
    ...
    if resume is None:
        raise HTTPException(status_code=404, detail='Резюме не найдено')
    if await db.resumes.has_active_assignment(resume_id):
        raise HTTPException(status_code=409, detail='Нельзя удалить резюме, пока оно принято в проект.')
