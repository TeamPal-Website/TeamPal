...
async def setup_profile_with_resume(client: AsyncClient):
    user_data = await register_and_verify(client, 'resume_owner@example.com', 'password123')
...
class TestSearchResumes:
    ...
    async def test_search_resumes_with_skill_ids(self, client: AsyncClient, skill_id: int):
        response = await client.get('/resumes', params=[('skill_ids', str(skill_id)), ('skill_ids', str(skill_id))])
        assert response.status_code == 200
...
class TestCreateResume:
    ...
    async def test_create_resume_no_auth_returns_401(self, client: AsyncClient):
        response = await client.post('/resumes', json={'role_type_id': 1, 'about_me': 'Experienced developer', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [1], 'experiences': []})
        assert response.status_code == 401

    async def test_create_resume_success(self, authenticated_client: AsyncClient, skill_id: int, role_id: int):
        response = await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Experienced developer', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': []})
    ...
    async def test_create_resume_with_skills(self, authenticated_client: AsyncClient, skill_id: int, role_id: int):
        response = await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Desc', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': []})
        assert response.status_code == 200

    async def test_create_resume_with_experiences(self, authenticated_client: AsyncClient, skill_id: int, role_id: int):
        response = await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Desc', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': [{'company_name': 'Tech Corp', 'role_type_id': role_id, 'start_date': '2020-01-01', 'end_date': '2023-01-01', 'description': 'Worked on projects'}]})
        print(f'DEBUG: {response.status_code} - {response.json()}')
        assert response.status_code == 200

    async def test_create_resume_experience_end_date_future_returns_422(self, authenticated_client: AsyncClient, skill_id: int, role_id: int):
        future_end = (date.today() + timedelta(days=30)).isoformat()
        response = await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Desc', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': [{'company_name': 'Tech Corp', 'role_type_id': role_id, 'start_date': '2020-01-01', 'end_date': future_end, 'description': 'Worked on projects'}]})
        assert response.status_code == 422

    async def test_create_resume_exceeds_limit(self, authenticated_client: AsyncClient, skill_id: int, role_id: int):
        for i in range(5):
            await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Desc', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': []})
        response = await authenticated_client.post('/resumes', json={'role_type_id': role_id, 'about_me': 'Desc', 'employment_intent': 'commercial', 'status': 'looking_for_job', 'skill_ids': [skill_id], 'experiences': []})
