...
async def _complete_profile(client: AsyncClient) -> None:
    r = await client.patch('/profiles', json=COMPLETE_PROFILE_JSON)
    assert r.status_code == 200, r.text

async def _catalog(client: AsyncClient, suffix: str) -> tuple[int, int]:
    role = await client.post('/roles_dictionary', json={'name': f'Backend {suffix}'})
    assert role.status_code == 200
    skill = await client.post('/skills', json={'name': f'Python {suffix}'})
    assert skill.status_code == 200
    return (role.json()['data']['id'], skill.json()['data']['id'])

async def _project(client: AsyncClient, token: str, role_id: int, title: str, vacancies_count: int=1) -> tuple[int, list[int]]:
    _auth(client, token)
    await _complete_profile(client)
    payload = {'title': title, 'company_name': 'TeamPal', 'employment_intent': 'commercial', 'description': 'Описание проекта', 'tasks': 'Задачи проекта', 'vacancies': [{'role_type_id': role_id, 'experience': 'none', 'work_format': 'remote', 'schedule': '5/2', 'commitment_level': 'full_time', 'description': 'Делать API\n\nЗнать FastAPI', 'salary_amount': 100000 + idx, 'salary_type': 'monthly', 'contract_type': 'gph'} for idx in range(vacancies_count)]}
    response = await client.post('/projects', json=payload)
    assert response.status_code == 200, response.text
    data = response.json()['data']
    return (data['project']['id'], [v['id'] for v in data['vacancies']])

async def _resume(client: AsyncClient, token: str, skill_id: int, role_type_id: int) -> int:
    _auth(client, token)
    await _complete_profile(client)
    payload = {'role_type_id': role_type_id, 'employment_intent': 'commercial', 'commitment_level': 'full_time', 'work_format': 'remote', 'schedule': '5/2', 'salary_amount': 120000, 'salary_type': 'monthly', 'contract_type': 'gph', 'about_me': 'Кандидат для workflow', 'skill_ids': [skill_id], 'experiences': []}
...
async def _my_application(client: AsyncClient, token: str, application_id: int) -> dict:
    _auth(client, token)
...
class TestApplicationsWorkflow:
    ...
    async def test_reject_blocks_same_user_for_same_vacancy(self, client: AsyncClient):
        ...
        _auth(client, applicant)
        response = await client.post('/applications', json={'resume_id': second_resume, 'vacancy_id': vacancies[0]})
    ...
    async def test_pending_cancel_and_project_paused(self, client: AsyncClient):
        ...
        _auth(client, owner)
        response = await client.patch(f'/projects/{project_id}', json={'status': 'paused'})
        assert response.status_code == 200
    ...
    async def test_delete_project_cancels_pending_and_blocks_occupied_delete(self, client: AsyncClient):
        ...
        pending_project, pending_vacancies = await _project(client, owner, role_id, 'Delete pending project')
        pending_resume = await _resume(client, applicant, skill_id, role_id)
        pending_application = await _apply(client, applicant, pending_resume, pending_vacancies[0])
        _auth(client, owner)
        ...
        occupied_project, occupied_vacancies = await _project(client, owner, role_id, 'Delete occupied project')
        occupied_resume = await _resume(client, applicant, skill_id, role_id)
        occupied_application = await _apply(client, applicant, occupied_resume, occupied_vacancies[0])
        _auth(client, owner)
    ...
    async def test_close_releases_resume_for_new_project(self, client: AsyncClient):
        ...
        first_project, first_vacancies = await _project(client, owner, role_id, 'Close first project')
        resume_id = await _resume(client, applicant, skill_id, role_id)
        ...
        _, second_vacancies = await _project(client, owner, role_id, 'Close second project')
        new_application_id = await _apply(client, applicant, resume_id, second_vacancies[0])
