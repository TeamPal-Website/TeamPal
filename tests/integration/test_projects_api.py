...
class TestCreateProject:
    ...
    async def test_create_project_no_auth_returns_401(self, client: AsyncClient):
        response = await client.post('/projects', json={'title': 'Test Project', 'company_name': 'Test Company', 'description': 'Description', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': 1, 'description': 'Test'}]})
        assert response.status_code == 401

    async def test_create_project_success(self, authenticated_client: AsyncClient, role_id: int):
        response = await authenticated_client.post('/projects', json={'title': 'Test Project', 'company_name': 'Test Company', 'description': 'Description', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Need developer'}]})
        assert response.status_code == 200

    async def test_create_project_with_vacancies(self, authenticated_client: AsyncClient, role_id: int):
        response = await authenticated_client.post('/projects', json={'title': 'Project with Vacancy', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Need developer'}]})
        assert response.status_code == 200

    async def test_create_project_invalid_city(self, authenticated_client: AsyncClient):
        response = await authenticated_client.post('/projects', json={'title': 'Project', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'city_id': 99999, 'vacancies': [{'role_type_id': 1, 'description': 'Test'}]})
        assert response.status_code == 404

    async def test_create_project_invalid_role(self, authenticated_client: AsyncClient):
        response = await authenticated_client.post('/projects', json={'title': 'Project', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': 99999, 'description': 'Need developer'}]})
        assert response.status_code == 404

    async def test_create_project_exceeds_limit(self, authenticated_client: AsyncClient, role_id: int):
        for i in range(10):
            await authenticated_client.post('/projects', json={'title': f'Project {i}', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Test'}]})
        response = await authenticated_client.post('/projects', json={'title': 'Extra Project', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': 1, 'description': 'Test'}]})
