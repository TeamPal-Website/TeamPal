...
async def setup_project_with_vacancy(client: AsyncClient):
    user_data = await register_and_verify(client, 'vacancy_owner@example.com', 'password123')
    login_resp = await client.post('/auth/login', json=user_data)
    token = login_resp.json()['access_token']
    client.cookies.set('access_token', token)
    prof = await client.patch('/profiles', json=COMPLETE_PROFILE_JSON)
    assert prof.status_code == 200
    role_resp = await client.post('/roles_dictionary', json={'name': 'Developer'})
    role_id = role_resp.json()['data']['id']
    project_resp = await client.post('/projects', json={'title': 'Test Project', 'company_name': 'Test Company', 'description': 'Description', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Need developer'}]})
...
class TestListProjectVacancies:
    ...
    async def test_list_vacancies_empty(self, authenticated_client: AsyncClient):
        ...
        role_id = role_resp.json()['data']['id']
        project_resp = await authenticated_client.post('/projects', json={'title': 'Empty Project', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Test'}]})
        project_id = project_resp.json()['data']['project']['id']
        vacancy_id = project_resp.json()['data']['vacancies'][0]['id']
...
class TestCreateProjectVacancy:
    ...
    async def test_create_vacancy_success(self, authenticated_client: AsyncClient):
        ...
        role_id = role_resp.json()['data']['id']
        project_resp = await authenticated_client.post('/projects', json={'title': 'Project for Vacancy', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Initial'}]})
        project_id = project_resp.json()['data']['project']['id']
        response = await authenticated_client.post(f'/projects/{project_id}/vacancies', json={'role_type_id': role_id, 'description': 'Looking for frontend developer'})
        assert response.status_code == 200
    ...
    async def test_create_vacancy_invalid_role(self, authenticated_client: AsyncClient):
        ...
        role_id = role_resp.json()['data']['id']
        project_resp = await authenticated_client.post('/projects', json={'title': 'Project', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Test'}]})
        project_id = project_resp.json()['data']['project']['id']
        response = await authenticated_client.post(f'/projects/{project_id}/vacancies', json={'role_type_id': 99999, 'description': 'Invalid role'})
        assert response.status_code == 404

    async def test_create_vacancy_project_not_found(self, authenticated_client: AsyncClient):
        response = await authenticated_client.post('/projects/99999/vacancies', json={'role_type_id': 1, 'description': 'Test'})
        assert response.status_code == 404

    async def test_create_vacancy_exceeds_limit(self, authenticated_client: AsyncClient):
        role_resp = await authenticated_client.post('/roles_dictionary', json={'name': 'Dev'})
        role_id = role_resp.json()['data']['id']
        project_resp = await authenticated_client.post('/projects', json={'title': 'Project with many vacancies', 'company_name': 'Company', 'description': 'Desc', 'tasks': 'Tasks', 'status': 'active', 'employment_intent': 'commercial', 'vacancies': [{'role_type_id': role_id, 'description': 'Initial'}]})
        project_id = project_resp.json()['data']['project']['id']
        for i in range(10):
            await authenticated_client.post(f'/projects/{project_id}/vacancies', json={'role_type_id': role_id, 'description': f'Vacancy {i}'})
        response = await authenticated_client.post(f'/projects/{project_id}/vacancies', json={'role_type_id': role_id, 'description': 'Extra vacancy'})
        assert response.status_code == 409

    async def test_create_vacancy_no_auth_returns_401(self, client: AsyncClient):
        response = await client.post('/projects/1/vacancies', json={'role_type_id': 1, 'description': 'Test'})
        assert response.status_code == 401
...
class TestUpdateProjectVacancy:
    ...
    async def test_update_vacancy_success(self, authenticated_client: AsyncClient):
        _, project_id, vacancy_id, _ = await setup_project_with_vacancy(authenticated_client)
        response = await authenticated_client.patch(f'/projects/{project_id}/vacancies/{vacancy_id}', json={'description': 'Updated description'})
        assert response.status_code == 200
    ...
    async def test_update_vacancy_not_found(self, authenticated_client: AsyncClient):
        ...
        response = await authenticated_client.patch(f'/projects/{project_id}/vacancies/99999', json={'description': 'Updated'})
        assert response.status_code == 404

    async def test_update_vacancy_project_not_found(self, authenticated_client: AsyncClient):
        response = await authenticated_client.patch('/projects/99999/vacancies/1', json={'description': 'Updated'})
        assert response.status_code == 404

    async def test_update_vacancy_no_auth_returns_401(self, client: AsyncClient):
        response = await client.patch('/projects/1/vacancies/1', json={'description': 'Updated'})
        assert response.status_code == 401
...
class TestDeleteProjectVacancy:
    ...
    async def test_delete_vacancy_success(self, authenticated_client: AsyncClient):
        _, project_id, vacancy_id, _ = await setup_project_with_vacancy(authenticated_client)
        response = await authenticated_client.delete(f'/projects/{project_id}/vacancies/{vacancy_id}')
        assert response.status_code == 200
    ...
    async def test_delete_vacancy_not_found(self, authenticated_client: AsyncClient):
        _, project_id, _, _ = await setup_project_with_vacancy(authenticated_client)
        response = await authenticated_client.delete(f'/projects/{project_id}/vacancies/99999')
        assert response.status_code == 404
