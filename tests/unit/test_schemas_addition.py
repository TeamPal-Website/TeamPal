...
from src.schemas.profiles import ProfileAdd, ProfileBase, ProfileContacts, ProfileRequestPatch
...
class TestResumeRequestAddSalary:
    ...
    def test_salary_at_max_ok(self):
        r = ResumeRequestAdd(role_type_id=1, salary_amount=MAX_SALARY_AMOUNT_RUB, skill_ids=[1], experiences=[])
        assert r.salary_amount == MAX_SALARY_AMOUNT_RUB

    def test_salary_above_max_fails(self):
        with pytest.raises(ValidationError):
            ResumeRequestAdd(role_type_id=1, salary_amount=MAX_SALARY_AMOUNT_RUB + 1, skill_ids=[1], experiences=[])
