...
class TestUser:
    ...
    def test_from_attributes(self):
        orm_obj = SimpleNamespace(id=1, email='test@example.com', is_active=True, hashed_password='secret')
...
class TestUserWithHashedPassword:
    ...
    def test_includes_hashed_password(self):
        user = UserWithHashedPassword(id=1, email='test@example.com', is_active=True, hashed_password='somehash')
        assert user.hashed_password == 'somehash'

    def test_inherits_user_fields(self):
        user = UserWithHashedPassword(id=5, email='admin@example.com', is_active=False, hashed_password='hash')
        assert user.id == 5
