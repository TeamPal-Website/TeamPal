...
TeamPal — платформа для поиска участников и формирования команд под IT-проекты: от учебного и pet-проекта до коммерческой команды. Job-порталы (hh.ru), карьерные сети и мессенджеры ориентированы на трудоустройство, а не на сбор команды под задачу.

В одном аккаунте доступны оба сценария:

- **Соискатель** (участник команды) — резюме, навыки, поиск проектов, отклики.
- **Создатель проекта** (инициатор) — карточка проекта, вакансии, заявки, формирование команды.
...
| База данных | PostgreSQL 16, pgvector (семантический поиск) |
...
Миграция `seed_roles_and_skills` заполняет базовые города, навыки и роли. Дополнительные значения — через API:
...
- До 5 резюме: **коммерческое** (зарплата, договор, формат работы) и **учебное**
...
### Создатель проекта (инициатор)

- Проекты **коммерческие** и **учебные (некоммерческие)** — до 10 на профиль; до 10 вакансий на проект
...
│   ├── unit/
│   └── integration/
├── docs/project/     # PDF: требования, внедрение, жизненный цикл, потребность в продукте
└── README.md
```
...
|--------|-----------|------------|---------|
| `nginx` | `team_pal_nginx` | Статика, прокси API, `/s3/`; rate limit 10 r/s (статика), 5 r/s (API) | http://localhost:8080 (`APP_PORT`) |
...
- Закрывайте или приостанавливайте проект после набора, чтобы не получать лишние заявки.
...
Реализовано (см. `docs/project/requirements.pdf`):

- Регистрация, JWT в cookie, профиль, резюме и проекты, поиск, отклики и приглашения, уведомления.
- **Рекомендации** — pgvector + гибридный score (embeddings, роль, навыки), кэш Redis, Celery.
- **Подтверждение email** — SMTP или код в логе `app` без SMTP.

Планируется: рейтинг и отзывы; расширение форматов (хакатон); тонкая настройка рекомендаций; масштабирование инфраструктуры.

---

## Production-деплой

Сайт: [https://team-pal.ru](https://team-pal.ru). Подробнее — `docs/project/deployment-guide.pdf`.

```bash
git clone https://github.com/TeamPal-Website/TeamPal.git
cd TeamPal/infra
cp .env.example .env   # пароли, JWT, ALLOWED_ORIGINS, домен
docker compose up -d --build   # подхватывает override: nginx + HTTPS
```

HTTPS: certbot (Let's Encrypt), домен и пути к сертификатам в `nginx.conf`, надёжные секреты в `.env`, закрыть внутренние порты (5433, 6379, 9000) в firewall.
